...
struct CRGBLPD {
    ...
    // default values are UNINITIALIZED
	inline CRGBLPD() __attribute__((always_inline))
    {
    }
	inline uint16_t package()  __attribute__((always_inline))
	{
		data = r & 0x1F;
		data <<= 5;
		data |= g & 0x1F;
    ...
    // allow construction from R, G, B
    inline CRGBLPD( uint8_t ir, uint8_t ig, uint8_t ib)  __attribute__((always_inline))
        : r(ir), g(ig), b(ib)
    {
    	data = r & 0x1F;
    	data <<= 5;
    ...
    // allow construction from 32-bit (really 24-bit) bit 0xRRGGBB color code
    inline CRGBLPD( uint32_t colorcode)  __attribute__((always_inline))
    : r((colorcode >> 16) & 0xFF), g((colorcode >> 8) & 0xFF), b((colorcode >> 0) & 0xFF)
    {
    	data = r & 0x1F;
    	data <<= 5;
    ...
    inline CRGBLPD( LEDColorCorrection colorcode) __attribute__((always_inline))
    : r((colorcode >> 16) & 0xFF), g((colorcode >> 8) & 0xFF), b((colorcode >> 0) & 0xFF)
    {
    	data = r & 0x1F;
    	data <<= 5;
    ...
    inline CRGBLPD( ColorTemperature colorcode) __attribute__((always_inline))
    : r((colorcode >> 16) & 0xFF), g((colorcode >> 8) & 0xFF), b((colorcode >> 0) & 0xFF)
    {
    	data = r & 0x1F;
    	data <<= 5;
    ...
    // allow copy construction
    inline CRGBLPD(const CRGB& rhs) __attribute__((always_inline))
        {
            r = rhs.r;
            g = rhs.g;
            b = rhs.b;
	...
	inline CRGBLPD(const CRGBLPD& rhs) __attribute__((always_inline))
    {
        r = rhs.r;
        g = rhs.g;
        b = rhs.b;
    ...
    // allow assignment from one RGBLPD struct to another
	inline CRGBLPD& operator= (const CRGBLPD& rhs) __attribute__((always_inline))
    {
        r = rhs.r;
        g = rhs.g;
        b = rhs.b;
	...
	// allow assignment from one RGB to RGBLPD
	inline CRGBLPD& operator= (const CRGB& rhs) __attribute__((always_inline))
	{
	    r = rhs.r;
	    g = rhs.g;
	    b = rhs.b;
    ...
    // allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
	inline CRGBLPD& operator= (const uint32_t colorcode) __attribute__((always_inline))
    {
        r = (colorcode >> 16) & 0xFF;
        g = (colorcode >>  8) & 0xFF;
        b = (colorcode >>  0) & 0xFF;
    ...
    // allow assignment from R, G, and B
	inline CRGBLPD& setRGB (uint8_t nr, uint8_t ng, uint8_t nb) __attribute__((always_inline))
    {
        r = nr;
        g = ng;
        b = nb;
    ...
    // allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
	inline CRGBLPD& setColorCode (uint32_t colorcode) __attribute__((always_inline))
    {
        r = (colorcode >> 16) & 0xFF;
        g = (colorcode >>  8) & 0xFF;
        b = (colorcode >>  0) & 0xFF;
    ...
    // add one RGB to another, saturating at 0xFF for each channel
    inline CRGBLPD& operator+= (const CRGBLPD& rhs )
    {
        r = qadd8( r, rhs.r);
        g = qadd8( g, rhs.g);
        b = qadd8( b, rhs.b);
    ...
    inline CRGBLPD& operator+= (const CRGB& rhs )
    {
        r = qadd8( r, rhs.r);
        g = qadd8( g, rhs.g);
        b = qadd8( b, rhs.b);
    ...
    // can't usefully decide when it's being passed a 32-bit
    // constant (e.g. CRGB::Red) and an 8-bit one (CRGB::Blue)
    inline CRGBLPD& addToRGB (uint8_t d )
    {
        r = qadd8( r, d);
        g = qadd8( g, d);
        b = qadd8( b, d);
    ...
    // can't usefully decide when it's being passed a 32-bit
    // constant (e.g. CRGB::Red) and an 8-bit one (CRGB::Blue)
    inline CRGBLPD& subtractFromRGB(uint8_t d )
    {
        r = qsub8( r, d);
        g = qsub8( g, d);
        b = qsub8( b, d);
    ...
    // subtract a constant of '1' from each channel, saturating at 0x00
    inline CRGBLPD& operator-- ()  __attribute__((always_inline))
    {
        subtractFromRGB(1);
        return *this;
    }
    ...
    // subtract a constant of '1' from each channel, saturating at 0x00
    inline CRGBLPD operator-- (int DUMMY_ARG)  __attribute__((always_inline))
    {
        CRGB retval(*this);
        --(*this);
        return retval;
    ...
    // divide each of the channels by a constant
    inline CRGBLPD& operator/= (uint8_t d )
    {
        r /= d;
        g /= d;
        b /= d;
    ...
    // nonzero, it'll stay nonzero, even if that means the hue shifts a little
    // at low brightness levels.
    inline CRGBLPD& nscale8_video (uint8_t scaledown )
    {
        nscale8x3_video( r, g, b, scaledown);
        data = r & 0x1F;
    	data <<= 5;
    ...
    // %= is a synonym for nscale8_video.  Think of it is scaling down
    // by "a percentage"
    inline CRGBLPD& operator%= (uint8_t scaledown )
    {
        nscale8x3_video( r, g, b, scaledown);
        data = r & 0x1F;
    	data <<= 5;
    ...
    // fadeLightBy is a synonym for nscale8_video( ..., 255-fadefactor)
    inline CRGBLPD& fadeLightBy (uint8_t fadefactor )
    {
        nscale8x3_video( r, g, b, 255 - fadefactor);
        data = r & 0x1F;
    	data <<= 5;
    ...
    // 'plain math' dimming rules, which means that if the low light levels
    // may dim all the way to 100% black.
    inline CRGBLPD& nscale8 (uint8_t scaledown )
    {
        nscale8x3( r, g, b, scaledown);
        data = r & 0x1F;
    	data <<= 5;
    ...
    // fadeToBlackBy is a synonym for nscale8( ..., 255-fadefactor)
    inline CRGBLPD& fadeToBlackBy (uint8_t fadefactor )
    {
        nscale8x3( r, g, b, 255 - fadefactor);
        data = r & 0x1F;
    	data <<= 5;
    ...
    inline CRGBLPD lerp8( CRGB & other, fract8 frac)
    {
      CRGBLPD ret;
    ...
    inline CRGBLPD lerp8( CRGBLPD & other, fract8 frac)
    {
      CRGBLPD ret;
      ret.r = lerp8by8(r,other.r,frac);
      ret.g = lerp8by8(g,other.g,frac);
...
inline __attribute__((always_inline)) bool operator== (const CRGBLPD& lhs, const CRGBLPD& rhs)
{
    return (lhs.r == rhs.r) && (lhs.g == rhs.g) && (lhs.b == rhs.b);
}
...
__attribute__((always_inline))
inline CRGBLPD operator+( const CRGBLPD& p1, const CRGBLPD& p2)
{
    return CRGBLPD( qadd8( p1.r, p2.r),
                 qadd8( p1.g, p2.g),
                 qadd8( p1.b, p2.b));
}
...
inline CRGBLPD operator-( const CRGBLPD& p1, const CRGBLPD& p2)
{
    return CRGBLPD( qsub8( p1.r, p2.r),
                 qsub8( p1.g, p2.g),
                 qsub8( p1.b, p2.b));
}

__attribute__((always_inline))
inline CRGBLPD operator*( const CRGBLPD& p1, uint8_t d)
{
    return CRGBLPD( qmul8( p1.r, d),
                 qmul8( p1.g, d),
                 qmul8( p1.b, d));
}
...
inline CRGBLPD operator/( const CRGBLPD& p1, uint8_t d)
{
    return CRGBLPD( p1.r/d, p1.g/d, p1.b/d);
}
...
inline CRGBLPD operator&( const CRGBLPD& p1, const CRGBLPD& p2)
{
    return CRGBLPD( p1.r < p2.r ? p1.r : p2.r,
                 p1.g < p2.g ? p1.g : p2.g,
                 p1.b < p2.b ? p1.b : p2.b);
}
